...
public class Selctmodel : MonoBehaviour
{
	...
	[SerializeField] private bool selectRootObject = true; // Select the root of the hit object
	[SerializeField] private bool includeChildrenForHighlight = true; // Highlight all child renderers
	...
	void Update()
	{
		...
		{
			if (logSelection) Debug.Log("Escape");
			Deselect();
			return;
		}
		...
		{
			if (logSelection) Debug.Log("Left click");
	...
	private void HandleClickSelect(Vector3 screenPosition)
	{
		...
		if (Physics.Raycast(ray, out RaycastHit hit, Mathf.Infinity, selectableLayers))
		{
			...
			GameObject target = selectRootObject ? hit.collider.transform.root.gameObject : hit.collider.gameObject;
			Select(target);
		}
		else
		{
			...
			{
				if (logSelection) Debug.Log($"Bounds pick hit: {hitRenderer.name} at distance {boundsDistance:F2}");
				VisualizeRay(ray, ray.origin + ray.direction * boundsDistance);
				GameObject target = selectRootObject ? hitRenderer.transform.root.gameObject : hitRenderer.gameObject;
				Select(target);
			}
			else if (deselectOnEmptyClick)
			{
				if (logSelection) Debug.Log("Deselecting");
	...
	private void ApplyHighlight(GameObject target)
	{
		...
		Renderer[] renderers = includeChildrenForHighlight
